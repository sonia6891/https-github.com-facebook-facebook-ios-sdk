...
・薪資單辨識與逐項對帳（Pro）
iPhone 使用 Apple Vision 在裝置端辨識公司薪資單，確認後帶入實際金額，逐項比較底薪、津貼、加班費與扣款，協助找出少發、多發、多扣或少扣。
...
Pro 薪資辨識與對帳：
iPhone 薪資單辨識使用 Apple Vision 在裝置端完成，並以欄位位置、同義詞、格式記憶與交叉比對整理金額。逐項薪資對帳會比較 App 預估與公司薪資單實際金額，標示少發、多發、多扣或少扣。這些功能不呼叫 OpenAI。
